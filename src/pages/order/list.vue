<template>
	<view class="tabs">
		<scroll-view class="scroll-h" :scroll-x="true" :show-scrollbar="false" :scroll-into-view="scrollInto">
			<view
v-for="(tab,index) in tabBars" :key="tab.status" class="uni-tab-item" :data-current="index"
				@click="ontabtap">
				<text
class="uni-tab-item-title"
		...
		<swiper :current="tabIndex" class="swiper-box" style="flex: 1;" :duration="300" @change="ontabchange">
			<swiper-item v-for="(tab,index1) in newsList" :key="index1" class="swiper-item">
				<scroll-view
class="scroll-v list" enable-back-to-top="true" scroll-y refresher-enabled
					:refresher-triggered="refeshloading" @refresherrefresh="refresh" @scrolltolower="loadMore(tab)">
					<view v-for="(item,index2) in tab.data" :key="index2" class="list-item">

						<view class="item-number">
							<text>订单编号：{{item.code}}</text>
							<text
								:class="{'done':item.orderStat==30,'toSend':item.orderStat==21,'toAudit':item.orderStat==10}">{{retrunStatusText(item.status)}}</text>
						</view>
							...
							<view v-for="(i,n) in item.items" :key="n" class="">
								<view class="flex">
									<view class="product-img">
										<image :src="i.pic" alt="">
											<!-- <image src="https://vkceyugu.cdn.bspapp.com/VKCEYUGU-dc-site/b2e201d0-517d-11eb-8a36-ebb87efcf8c0.jpg" alt=""> -->
									</view>
									<view class="list-item-content-right flex-sub">
										...
										<view class="product-price">
											<text
												class="l">总价：¥{{item.payMoney}}，优惠：¥{{item.payMoney - item.actualMoney}}
											</text>
						...
						<view class="option flex justify-end">
							...
							<template
								v-if="item.status== 2 && item.subStatus !== 7 && item.subStatus !== 8 && item.subStatus !== 9">
...
import { ref, getCurrentInstance, reactive, toRef, computed, defineComponent, toRefs } from 'vue'
	...
	export default defineComponent({
		...
		methods: {
			...
			// 去分享
			toShare(item) {console.log('item', item)
